...
ValueNotifier<List<Item>> cart = ValueNotifier<List<Item>>([]);
...
class _MyCatalogState extends State<MyCatalog> {
  void refreshPage(ValueNotifier<List<Item>> _cart) =>
...
class __AddButtonState extends State<_AddButton> {
  ...
  Widget build(BuildContext context) {
    return ValueListenableBuilder(
      ...
      builder: (context, _cartList, child) {
        bool isInCart = _cartList.contains(widget.item);
        return TextButton(
          onPressed: isInCart
              ? null
              : () {
                  cart.value = [...cart.value, widget.item];
                },
          style: ButtonStyle(
...
class _MyAppBar extends StatelessWidget {
  ...
  final Function(ValueNotifier<List<Item>> _cart) refreshPage;
  @override
  Widget build(BuildContext context) {
    return SliverAppBar(
