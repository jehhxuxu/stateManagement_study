...
class _MyCartState extends State<MyCart> {
  ...
  Widget build(BuildContext context) {
    final ValueNotifier<List<Item>> cart =
          ...
          children: [
            Expanded(
              child: Padding(
                ...
                child: _CartList(
                  cart: cart,
                  onPressed: (index) => onPressed(index, cart.value),
                ),
              ),
            ),
            Divider(height: 4, color: Colors.black),
            _CartTotal(cart: cart.value)
          ],
        ),
      ),
    );
...
class __CartListState extends State<_CartList> {
  ...
  Widget build(BuildContext context) {
    ...
    return ListView.builder(
      itemCount: widget.cart.value.length,
